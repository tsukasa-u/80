...
if __name__ == "__main__":
    ...
    while True:
        ...
        for p_i in p:
            if p_i[0].is_alive():
                active_modual_list.append(p_i[1])
                ...
                for i, conn_data_li in enumerate(conn_data[mod_li]):
                    ...
                    if main_method == UNI:
                        if modual in active_modual_list:
                            ...
                            if task_name=="GetModualList":
                            ...
                            elif task_name=="RequestactiveModual":
                                ver_list = active_modual_list.copy()
                            ...
                            elif task_name=="RequestModualEnd":
                                sub_modual, = conn_info
                                del conn_data[mod_li][i]
                                for i in range(MAX_PROCESS):
                                    if sub_modual==p[i][1] and p[i][0].is_alive():
                                        conn_r, conn_w = connection[sub_modual]
                                        # conn_r.close()  #?
                                        # conn_w.close()  #?
                                        # del conn_r
                                        # del conn_w
                                ...
                                sub_modual, sub_args = conn_info
                                sub_func = METHOD[sub_modual]
                                del conn_data[mod_li][i]
