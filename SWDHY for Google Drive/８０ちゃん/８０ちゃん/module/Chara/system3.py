# import P_Ver
import module.Chara.P_Ver as P_Ver
...
class Charactor(tkinter.Frame):
    ...
    def set_time_event(self, func, name, handle, time, count, *args):
        ...
        if not count:
            count = 1
        if not self.module in self.time_event.keys():
            self.time_event[self.module] = {}
        self.time_event[self.module][name] = self.Chara_Time_Event(handle, time, count, func, *args)
        return self 

    def set_button_event(self, color, func, name, *args):
        if not self.module in self.button_event.keys():
            self.button_event[self.module] = {
                (i,j) : {} for i in ["Default", "Left", "Wheel", "Right"] for j in ["Press", "Motion", "Release", "Double", "Enter", "Leave"]
            }
        self.button_event[self.module][name][color] = self.Chara_Button_Event(color, func, *args)
        return self 

    def set_image(self, module, path, number, delete, image, centerpoint=None):
        if not module in self.img_li.keys():
            self.img_li[module] = {}
        if path:
            if centerpoint:
                for li, path_li, dele, point in zip(number, path, delete, centerpoint):
                    self.img_li[module][li] = self.Chara_Img(path_li, li, dele=dele, width=self.width, height=self.height, centerpoint=point)
                return self
            else:
                for li, path_li, dele in zip(number, path, delete):
                    self.img_li[module][li] = self.Chara_Img(path_li, li, dele=dele, width=self.width, height=self.height, centerpoint=None)
                return self
        else:
            if image:
                if centerpoint:
                    for li, image_li, dele, point in zip(number, image, delete, centerpoint):
                        self.img_li[module][li] = self.Chara_Img(None, li, dele=dele, width=self.width, height=self.height, image=image_li, centerpoint=point)
                    return self
                else:
                    for li, image_li, dele in zip(number, image, delete):
                        self.img_li[module][li] = self.Chara_Img(None, li, dele=dele, width=self.width, height=self.height, image=image_li)
                    return self
            else:
                raise Exception("Error : cannot find image data")
            

    def set_motion(self, module, motion, move, repeat=False):
        self.count = -1
    ...
    def set_interupt_motion(self, module, motion, move, button, repeat=False):
        self.interupt_count = -1
    ...
    def load_image(self, module, num=None):
        if num==None:
            for im_li in self.img_li[module].values():
                im_li.read()
        else:
            self.img_li[module][num].read()
        return self

    def set_bg(self, module, path=None, image=None, num=None):
        # if self.img_li[module][-1]:
        #     del self.img_li[module][-1]
        if num==None:
            self.img_li[module][-1] = self.Chara_Img(path=path, num=-1, dele=True, width=self.width, height=self.height, image=image)
        else:
            self.img_li[module][-1] = copy.copy(self.img_li[module][num])
        return self
    ...
    def get_pixel(self, x, y):
        if -1 in self.img_li[self.module].keys():
            if 0<x<self.width:
                if 0<y<self.height:
                    self.rgb = self.img_li[self.module][-1].image_resized.getpixel((x, y))
    ...
    def select_func(self, event, name):
        ...
        if name[1]=="Enter" or name[1]=="Leave":
            if self.module in self.button_event:
                if None in self.button_event[self.module][name].keys():
                    eve = self.button_event[self.module][name][None]
                    eve.func(event, *eve.args)
        else:
            self.x, self.y = event.x, event.y
            color = self.get_pixel(self.x, self.y)
            if self.module in self.button_event:
                if color in self.button_event[self.module][name].keys():
                    eve = self.button_event[self.module][name][color]
                    eve.func(event, *eve.args)
    ...
    def get_info(self):
        if self.connect_read.poll():
            ...
            for i, con_data_li in enumerate(conn_data):
                task_name, main_method, sub_method, module, *conn_info = con_data_li
                if task_name=="SetMotion":
                    del conn_data[i]
                    self.connect_write.send(conn_data)
                    self.module = module   #   果たしてselfでよいのか
                    repeat, motion, move = conn_info

                    self.set_motion(self.module, motion, move, repeat=repeat)
                    ...
                    self.set_interupt_motion(self.module, motion, move, ope, repeat=repeat)
                elif task_name=="SetPath":
                    del conn_data[i]
                    self.connect_write.send(conn_data)
                    path, number, delete = conn_info

                    self.set_image(module=module, path=path, number=number, delete=delete, image=None)
                elif task_name=="GetCharaPos":
                    del conn_data[i]
                    conn_data.append([
                        "GetCharaPos",
                        "UNI",
    ...
    def convert_data(self, conn_data, data):
        ...
        #         "SEND",
        #         "Pallet",
        #         (self.x-self.pre_x, self.y-self.pre_y)
        #     ]
        # ])

        # print("\r", self.root_x, self.root_y, end="")
        ...
        if self.event_occurence in ["Left", "Wheel", "Right"]:
                ...
                if self.interupt_mo_sche[button] and self.event_occurence==button:
                    if not(self.interupt_count>=self.interupt_mo_sche_len[button]-1 and self.interupt_is_repeat[button]==False):
                        ...
                        if previous_image!=current_image or self.interupt_mo_sche_len==1:
                            ...
                            self.id = self.canvas.create_image(0, 0, image=self.img_li[self.module][current_image].im, anchor='nw')
                        if self.img_li[self.module][current_image].centerpoint:
                            center_x, center_y = self.img_li[self.module][current_image].centerpoint
            ...
            if not(self.count>=self.motion_sche_len-1 and self.is_repeat==False):
                ...
                if previous_image[2]!=current_image[2] or self.motion_sche_len==1:
                    self.canvas.delete(self.id)
                    # print("\r"+str(self.img_li[self.module]), end="")
                    self.id = self.canvas.create_image(0, 0, image=self.img_li[self.module][current_image[2]].im, anchor='nw')
