...
def hello(conn_r, conn_w) :
    ...
    if os.path.exists(PATH+"/todo.dat")== True:
        pass
    else:
        f = pathlib.Path(PATH+"/todo.dat")
        f.touch()

    for line in open(PATH+"/todo.dat",encoding="utf-8_sig"):
        a = eval(line[:-1])
        ...
        if not comp_list[yy][mm][dd]:
            ...
            def LBSC(evt):
                def delsc_click():
                    for i in sche8.curselection():
                        with open(PATH+"/todo.dat", encoding="utf-8_sig") as f:
                            todot = f.read()
                        todot = todot.replace('"','')
                        ...
                        with open(PATH+"/todo.dat", mode="w", encoding="utf-8_sig") as f:
                            f.write(todot)
            ...
            def nsc_click():
                def ok1_click():
                    ...
                    with open(PATH+'/todo.dat', 'a',newline="\n",encoding="utf-8_sig") as f:
