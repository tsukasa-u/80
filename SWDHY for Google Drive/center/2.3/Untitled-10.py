...
class Charactor(tkinter.Frame):
    ...
    class Chara_Img():
        def __init__(self, path, num, dele=True, width=None, height=None):
            self.number = num
            self.path = path
            self.width = width
            self.height = height
    ...
    class Chara_Event():
        ...
        def exert(self):
            self.handle.after(self.time, self.func, *self.args, **self.kargs)
            if self.count>1:
                self.count -=1
    ...
    def set_time_event(self, func, name, handle=None, time=16, count=1, args=[], kargs={}):
        if not handle:
            handle = self.master
        self.event[name] = self.Chara_Event(handle, time, count, func, args, kargs)
        return self 
    ...
    def frame(self):
        ...
        if not self.event_occurence:
            ...
            previous_image = self.interupt_mo_sche[self.count%self.interupt_mo_sche_len]
            self.count += 1
            current_image = self.interupt_mo_sche[self.count%self.interupt_mo_sche_len]
            if previous_image[2]!=current_image[2]:
                self.canvas.delete(self.id)
                self.id = self.canvas.create_image(0, 0, image=self.img_li[self.modual][current_image[2]].im, anchor='nw')
...
if __name__=="__main__":
    ...
    Chara.set_interuot_motion(DEFAULT, motion, move, repeat=True)
