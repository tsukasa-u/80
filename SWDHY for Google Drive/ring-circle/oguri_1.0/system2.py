...
class Charactor(tkinter.Frame):
    def __init__(self,master, path, number, motion, move, dele, connect_write, connect_read, image=None, width=128, height=128, root_x=0, root_y=0):
        ...
        self.master.geometry("+"+str(self.root_x)+"+"+str(self.root_y))
        
        self.set_image(self.DEFAULT, self.path, self.number, dele, self.image)
    ...
    def set_bg(self, modual, path=None, image=None, num=None):
        ...
        if num==None:
            ...
            # print("####", self.img_li[modual], "---------------------------------------")
            # print("$$$$$", self.img_li[modual][num])
            self.img_li[modual][-1] = copy.copy(self.img_li[modual][num])
            # print("####", self.img_li[modual][-1], "--------------------------------------")
        return self
    ...
    def get_info(self):
        if self.connect_read.poll():
            ...
            if name=="SetMotion":
                ...
                self.set_interupt_motion(self.modual, motion, move, ope[0], repeat=repeat)
            elif name=="SetPath":
                info = info["info"]
                path = info["path"]
                number = info["number"]
